...
export default new Vuex.Store({
  ...
  actions: {
    load_notes({commit}){
      axios.get(`https://api-laravel-notes.herokuapp.com/api/notes`)
          .then(response => {
    ...
    delete_notes({commit} , id){
       axios.delete(`https://api-laravel-notes.herokuapp.com/api/notes/${id}`)
           .then(response =>{
    ...
    add_note({commit}, note){
      axios.post(`https://api-laravel-notes.herokuapp.com/api/notes`, note)
          .then(response => {
    ...
    update_note({commit}, note){
      axios.put(`https://api-laravel-notes.herokuapp.com/api/notes/${note.id}`, note)
          .then(response => {
            console.log(response.data.note)
           // commit('ADD_NOTE', response.data.Note)
          })
